...
用户注册
* */
router.post('/register_pwd', function (req, res) {
  const name = req.body.name
  const pwd = md5(req.body.pwd)
  UserModel.findOne({name}, function (err, user) {
    if (user) {
      // 返回提示错误信息
      res.send({code: 1, msg: '此用户已存在test'})
    } else {
...
router.post('/login_pwd', function (req, res) {
  ...
  console.log('/login_pwd', name, pwd, captcha, req.session.captcha, req.session)

  // 可以对用户名/密码格式进行检查, 如果非法, 返回提示信息
  // if (captcha !== req.session.captcha) {
  //   return res.send({code: 1, msg: '验证码不正确 test'})
  // }
  // 删除保存的验证码
